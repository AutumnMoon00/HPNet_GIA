...
def pca_torch(X):
    ...
    # The covariance is symmetric and only 3x3; solve it on CPU.
    return torch.linalg.eigh(covariance.cpu())
...
def standardize_point_torch(point, weights):
    ...
    R = torch.from_numpy(R).to(point.device).detach()
